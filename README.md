...
Он запускается каждый день в `03:00`, `08:00` и `11:00 UTC`, то есть в `08:00`, `13:00` и `16:00` по Екатеринбургу, обновляет данные, генерирует `index.html` и коммитит результат через `GITHUB_TOKEN`. Локальная авторизация GitHub на этой машине не используется.
